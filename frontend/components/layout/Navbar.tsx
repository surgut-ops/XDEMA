  ...
  return (
    <>
      <nav className={`fixed top-0 left-0 right-0 z-[800] h-[66px] flex items-center justify-between px-4 sm:px-10 transition-all duration-300 border-b ${scrolled ? 'border-white/10' : 'border-transparent'}`}
        ...
        <Link href="/" className="font-syne font-extrabold tracking-[.18em] text-[1.1rem] bg-gradient-to-r from-[var(--c1)] to-[var(--c2)] bg-clip-text text-transparent">
          XDEMA
        </Link>
        ...
        <div className="flex items-center gap-2">
          ...
          <button
            ...
            className="flex items-center gap-1.5 text-black font-bold text-xs tracking-wide px-3.5 py-1.5 rounded-[9px] transition-all hover:opacity-90 hover:-translate-y-px"
          ...
          <button
            onClick={handleAuth}
            className="flex items-center gap-1.5 px-3.5 py-1.5 rounded-[9px] text-xs font-medium transition-all hover:border-[var(--c1)] hover:text-[var(--c1)]"
          ...
          {/* Burger mobile */}
          <button className="flex sm:hidden flex-col gap-[5px] p-2" onClick={() => setMob(!mob)}>
            <span className={`block w-[22px] h-[2px] rounded-sm transition-all duration-300 ${mob ? 'rotate-45 translate-y-[7px]' : ''}`} style={{ background: 'var(--text)' }} />
            <span className={`block w-[22px] h-[2px] rounded-sm transition-all duration-300 ${mob ? 'opacity-0' : ''}`} style={{ background: 'var(--text)' }} />
            <span className={`block w-[22px] h-[2px] rounded-sm transition-all duration-300 ${mob ? '-rotate-45 -translate-y-[7px]' : ''}`} style={{ background: 'var(--text)' }} />
          </button>
        </div>
      </nav>
